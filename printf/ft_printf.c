...
static int	ft_hexa(long i, char *base, int n)
{
	char	*c;
	int		aux;
	int		baits;
...
static int	put(char a, va_list str)
{
	...
	if (a == '%')
		...
		baits = ft_hexa(va_arg(str, int), "0123456789abcdef", 0);
	else if (a == 'X')
		baits = ft_hexa(va_arg(str, int), "0123456789ABCDEF", 0);
	else if (a == 'p')
		baits = ft_hexa(va_arg(str, long), "0123456789abcdef", 4);
	return (baits);
}
